...
**AuraSub** 是基于原开源项目 VideoLingo 衍生出的一个强化分支。与原项目相比，AuraSub 最大的核心区别在于：**引入了基于大语言模型（LLM）的智能断句、基于发音同源的 ASR 语音纠错，以及高度优化的术语翻译机制。**

原项目在使用传统 NLP 算法进行句子切分时，遇到专业长难句极易断句错乱。AuraSub 彻底抛弃了物理切片，采用 LLM 进行上下文逻辑断句，在断句切分的合理性上比原项目**实现了准确率与上下文连贯性的跨越式提升**。此外，针对专业领域的错位发音，AuraSub 能在不全量灌入破坏上下文的前提下，精准替换底层词汇，从而为你提供绝对专业、无断层翻译的硬核字幕体验。

### 核心区别与特性：
- 🎥 **基础架构：** 继承自 VideoLingo 优秀的框架。
- **🧠 纯正 LLM 智能断句：** 放弃 SpaCy 等机械分词。利用大模型对毫无标点的听写稿进行标点推断与逻辑从句切分，断句质量有了质的飞跃。
- **🗣️ ASR 语音纠错引擎：** 独创发音纠错流程，对于 Whisper 听错的专业术语（如将 "two" 听成 "to"），系统会基于你提供的自定义术语表进行正则级扫描并自动修复原词。
...
- **🧠 双层级推理控制：** 支持在 WebUI 中直接为“复杂任务”（句法切分、翻译）和“简单任务”（直接翻译、总结）配置不同的推理强度 (Reasoning Effort)，最大化翻译性价比。
- **🎨 字幕样式与即时预览：** 新增专门的字幕自定义模块，支持动态调节中英文字号。采用合并的 ASS 方案实现无重叠、无空隙的紧凑双语排版，并在不重复翻译流程的情况下秒级提取最长句的单帧画面进行效果确认。
