...
### 2. **TTS 的 API**
AuraSub提供了多种 tts 接入方式，以下是对比（如不使用配音可跳过）
...
2. 将 `GPT-SoVITS-v2-xxx` 与 `AuraSub` 放在同一个目录下。**注意是两文件夹并列。**
...
3. 选择以下任一方式配置模型：
   ...
   - 在 AuraSub 网页的侧边栏中，将 `GPT-SoVITS 角色` 配置为 `你喜欢的英文角色名`。
...
配置完成后，注意在网页侧边栏选择 `参考音频模式`（具体原理可以参考语雀文档），AuraSub 在配音步骤时会自动在弹出的命令行中打开 GPT-SoVITS 的推理 API 端口，配音完成后可手动关闭。注意，此方法的稳定性取决于选择的底模。</details>

## 🛠️ 快速上手

AuraSub 支持 Windows、macOS 和 Linux 系统，可使用 CPU 或 GPU 运行。
...
开始安装 AuraSub 之前，请确保安装了 Git 和 Anaconda。
...
3. （可选）应用汉化补丁：
    ...
    将 `i18n/中文` 文件夹下的所有文件复制到 `AuraSub` 目录下，替换重复的文件。
